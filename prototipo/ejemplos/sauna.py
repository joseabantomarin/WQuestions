"""Dominio sauna — modelado completo en WQuestions.

Modela el dominio que el usuario describió en `dominios.md` (Dominio 9):
- Clientes, recepcionistas, sauna.
...
- Promociones combinadas (sauna + masaje).
- Programa de fidelidad: 7 sesiones → 1 gratis.
- Plan mensual con 2 sesiones de sauna por semana incluidas.
...
# ---------------------------------------------------------------------------
# 1. Configurar lexicon con verbos del dominio sauna
# ---------------------------------------------------------------------------
...
def build_lexicon() -> Lexicon:
    ...
    lex.register(LexiconEntry(
        ...
        aliases={
            ...
            "lugar_destino": ["a la cámara", "al sauna"],
        },
    ...
    lex.register(LexiconEntry(
        ...
        pattern=("sesion",),
        example="Ana tomó una sesión de sauna",
    ))
...
def build_universe(lex: Lexicon) -> Tuple[Universe, dict]:
    cat = Catalog()
    u = Universe(name="sauna_oasis", catalog=cat)
    ...
    # Lugares L
    sauna_central = u.add_individual(Individual(id="sauna_oasis_central", axis=Axis.L, label="Sauna Oasis (sede central)"))
...
def run_validations(u: Universe, lex: Lexicon, h: dict) -> List[Tuple[str, bool, str]]:
    ...
    results = []
    servicio_sauna = u.ind("servicio_sauna")
    ...
    sesiones = query(u, Pattern(
        ...
        type_constraint=servicio_sauna,
    ))
    n = len(sesiones)
    results.append((
    ...
    ok_pol = (
        e_sesion is not None and e_sesion.situation_type == "servicio_sauna"
...
def main():
    ...
    print("=" * 72)
    print("DOMINIO SAUNA — VALIDACIÓN DEL MODELO WQUESTIONS")
    print("=" * 72)
    print()
    print(u.summary())
    print()
