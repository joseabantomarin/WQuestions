...
La intuición que comienza a emerger es que los cuatro pilares son **estrictamente necesarios, pero arquitectónicamente insuficientes**. Necesitamos incorporar, como mínimo, una quinta pregunta —*cuál*, es decir, qué tipo de cosa es esto— que exija un eje propio y exclusivo: el zócalo categórico donde habitarán los tipos, las familias y los conceptos abstractos. A esto debemos sumarle una sexta pregunta para las métricas —*cuánto*— que lidiará con sus propias trampas de conversión (como las unidades de medida); y finalmente, una séptima pregunta —*cómo*— encargada de articular las conexiones y atributos que amarran todo el modelo. 

De esas cuatro preguntas restantes, la primera y más delicada de todo el modelo es el **eje categórico**: el zócalo donde habitan los tipos, las familias y los conceptos abstractos. Es, paradójicamente, el eje que soporta la mayor carga de trabajo en los sistemas del mundo real, y es aquí donde la arquitectura de WQuestions se integra pacíficamente con los gigantes de la industria (Schema.org, QUDT, SNOMED, CIDOC CRM), utilizándolos como aliados en lugar de intentar competir contra ellos: el vocabulario común que nos asegura no volver a construir, nunca más, la torre de Babel de los datos.