# Capítulo 12 — El verbo: La firma de contrato de la oración
...
Esto es exactamente lo que vimos en el Capítulo 10 cuando hablamos de la **reificación**. Nuestro modelo convierte las acciones en objetos del eje `O`. Y luego, usa hechos atómicos (tripletas) para enchufarle el agente, el lugar y el tiempo. 
...
A continuación va el catálogo entero, agrupado por familias. Para cada rol verás su nombre canónico, su signatura (qué cajas conecta) y para qué sirve en la práctica. La notación `O → Q` se lee como *"el sujeto del hecho vive en O y el valor del hecho vive en Q"*. Cuando un rol es **funcional** (`P`), un sujeto solo puede tener un valor (una consulta médica tiene un solo paciente, una sola fecha). Cuando es **no funcional** (`M`), un sujeto admite múltiples valores (una receta tiene varios ingredientes; un evento puede tener varias causas).
...
|---|---|---|
| `instancia_de` | O → K (M) | Marca que un objeto concreto es un ejemplar de una categoría. *"esta consulta es una instancia de `accion_consultar`"*. |
| `subtipo_de` | K → K (M) | Construye jerarquías de categorías. *"`jugador_de_futbol` es subtipo de `atleta_profesional`"*. |
| `parte_de` | O → O (M) | Indica que algo forma parte de un evento o entidad mayor. *"esta prescripción es parte de la consulta del 14 de mayo"*. |
| `contiene` | O → O (M) | El inverso del anterior. *"la consulta contiene esta prescripción"*. |
...
| Rol | Signatura | Para qué sirve |
|---|---|---|
| `agente` | O → Q (P) | Quien ejecuta la acción de forma intencional. *"María vendió"* → agente es María. |
| `paciente` | O → Q (P) | Quien recibe o sufre la acción. *"el médico atendió al paciente"*. |
| `tema` | O → O (P) | El objeto o sub-situación sobre el que recae la acción. *"prescribió un medicamento"*. |
| `beneficiario` | O → Q (P) | El destinatario o quien se beneficia. *"le dio un libro a María"* → María es beneficiaria. |
| `experimentador` | O → Q (P) | Quien vive un estado mental. *"Juan teme"*, *"Marta cree"*. Distingue al sentidor del agente activo. |
| `instrumento` | O → O (P) | El objeto que media la acción. *"pateó con la pierna izquierda"*, *"escribió con un bolígrafo"*. |
| `comprador` | O → Q (P) | Comprador en una venta — alias especializado de beneficiario. |
| `cliente` | O → Q (P) | Cliente de un servicio — alias frecuente en negocios. |

### Familia 3: Lugar — el "dónde"

| Rol | Signatura | Para qué sirve |
|---|---|---|
| `lugar_de` | O → L (P) | Lugar donde ocurre la situación. *"la consulta fue en el consultorio 03"*. |
| `origen` | O → L (P) | Punto de partida en movimientos o transferencias. |
| `destino` | O → L (P) | Punto de llegada. *"el taxi va al aeropuerto"*. |
| `lugar_destino` | O → L (P) | Alias de destino, útil cuando el verbo lo exige explícito (`ingresar`). |

### Familia 4: Tiempo — el "cuándo"

| Rol | Signatura | Para qué sirve |
|---|---|---|
| `momento` | O → T (P) | Instante puntual. *"firmó el contrato el 14 de mayo a las 10:30"*. |
| `inicio` | O → T (P) | Comienzo de un rango temporal. *"el préstamo vigente desde junio"*. |
| `fin` | O → T (P) | Fin de un rango temporal. Junto con `inicio` arma la bitemporalidad (D6). |

### Familia 5: Cantidad — el "cuánto"

| Rol | Signatura | Para qué sirve |
|---|---|---|
| `monto` | O → N (P) | Cantidad numérica con unidad. *"costó 49.90 dólares"*. |
| `cantidad` | O → N (P) | Alias de monto, más natural en contextos físicos (gramos, litros). |
| `por_cuanto` | O → N (P) | Precio o medida unitaria asociada. *"a 12 dólares la docena"*. |
| `unidad` | O → K (P) | Unidad de medida — siempre apunta a una categoría QUDT. |
...
| Rol | Signatura | Para qué sirve |
|---|---|---|
| `estatus_factual` | O → K (P) | Marca si la situación es `real`, `intencionada`, `planeada`, `cancelada`, `hipotetica`. |
| `modalidad` | O → K (P) | Tipo de modalidad: volitiva (deseo), deóntica (deber), alética (necesidad), epistémica (conocimiento). |
| `polaridad` | O → K (P) | Afirmativa o negativa. Permite registrar *"el paciente NO tiene fiebre"* como un hecho positivo. |
| `calificacion` | O → K (P) | Atributo cualitativo libre. *"clasificación de riesgo: alto"*. |

### Familia 7: Las cuatro relaciones del "por qué" (D7)

Ya las estudiamos en el capítulo 11, pero las incluimos aquí porque son parte oficial del catálogo.

| Rol | Signatura | Para qué sirve |
|---|---|---|
| `causado_por` | O → O (M) | Causalidad mecánica/física. *"el incendio causó el derrumbe"*. |
| `motivado_por` | O → O (M) | Motivo intencional. *"vendió la casa motivada por el deseo de mudarse"*. |
| `con_finalidad` | O → O (M) | Propósito teleológico — apunta a un estado futuro. *"abrió la vía con finalidad de acceder al pulmón"*. |
| `justificado_por` | O → O (M) | Autoridad normativa. *"la rescisión está justificada por la cláusula 14"*. |
...
| Rol | Signatura | Para qué sirve |
|---|---|---|
| `precede` | O → O (M) | Orden lógico o temporal. *"la aceptación precede al traslado"*. |
| `sigue_a` | O → O (M) | Inverso de precede. |
| `cumple` | O → O (M) | Una situación cumple una obligación o regla previa. |
| `cancela` | O → O (M) | Deja sin efecto una situación anterior. *"la cancelación cancela la reserva"*. |
| `rectifica` | O → O (M) | Corrige una situación previa sin borrarla (nunca borramos: rectificamos). |
| `contrasta_con` | O → O (M) | Relación adversativa, equivalente al "pero" del lenguaje. *"el paciente mejoró, pero contrasta con la elevación de la presión"*. |
...
| Rol | Signatura | Para qué sirve |
|---|---|---|
| `nombre` | Q → K (P) | Nombre legible de un agente. *"el agente `q_447` tiene nombre `María Gonzales`"*. |
| `identificador` | Q → K (P) | Identificador documental (DNI, RUC, número de empleado). |
...
(adopcion_001, lugar_de,        refugio_patitas_del_norte)   ∈ P(O, L)
(adopcion_001, momento,         2026-05-16T10:00:00Z)        ∈ P(O, T)
(adopcion_001, motivado_por,    pedido_mariana_001)          ∈ M(O, O)
(adopcion_001, estatus_factual, real)                        ∈ P(O, K)
```
...
(pedido_mariana_001, inicio,          2026-01-15)            ∈ P(O, T)
```
...
(multa_001, lugar_de,          av_central_km_12)             ∈ P(O, L)
(multa_001, momento,           2026-05-12T08:43:00Z)         ∈ P(O, T)
(multa_001, monto,             180)                          ∈ P(O, N)
(multa_001, unidad,            dolar_estadounidense)         ∈ P(O, K)
...
(infraccion_velocidad_001, monto,         92)                       ∈ P(O, N)
(infraccion_velocidad_001, unidad,        km_por_hora)              ∈ P(O, K)
```
...
(devolucion_001, momento,         2026-05-14T14:22:00Z)       ∈ P(O, T)
...
(reembolso_001, monto,            28.50)                      ∈ P(O, N)
(reembolso_001, unidad,           dolar_estadounidense)       ∈ P(O, K)
...
(resumen_ia_001, momento,         2026-05-10T15:58:00Z)        ∈ P(O, T)
...
(consulta_001, lugar_de,          ubicacion_remota)            ∈ P(O, L)
(consulta_001, momento,           2026-05-10T16:00:00Z)        ∈ P(O, T)
(consulta_001, modalidad,         remota_sincrona)             ∈ P(O, K)
(consulta_001, estatus_factual,   real)                        ∈ P(O, K)
...
(accidente_001, lugar_de,        obra_residencial_norte_03)   ∈ P(O, L)
(accidente_001, momento,         2026-05-07T11:40:00Z)        ∈ P(O, T)
(accidente_001, causado_por,     desprendimiento_cable_001)   ∈ M(O, O)
(accidente_001, estatus_factual, real)                        ∈ P(O, K)

(desprendimiento_cable_001, instancia_de,  evento_fisico)      ∈ M(O, K)
(desprendimiento_cable_001, tema,          cable_suelto_grua_torre_3) ∈ M(O, O)
(desprendimiento_cable_001, momento,       2026-05-07T11:40:00Z)      ∈ P(O, T)
...
Para que veas que esto no es un truco arreglado, apliquemos este mismo proceso "tonto y mecánico" a una industria totalmente distinta. Una clienta en un Sauna:
...
*(Dato curioso: Las palabras que usamos para preguntar en español descienden directamente de las reglas judiciales de Cicerón en Roma que vimos en el Capítulo 2. Los filósofos modernos lo redescubrieron hace poco, pero la mente humana ya lo traía de fábrica).*
...
Y eso es exactamente lo que el próximo capítulo te va a mostrar. No como un aburrido apéndice gramatical, sino como **el motor traductor definitivo** que le permite a una Inteligencia Artificial hablar directamente con los discos duros de tu empresa usando lenguaje humano.