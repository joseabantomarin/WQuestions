# Capítulo 19 — Un software ERP: el caso multi-módulo

## Por qué un ERP estresa al modelo

Hasta acá la Parte V modeló tres negocios — un Spa, un servicio de taxi, una clínica — y una pieza más exigente, un banco regional. Cada uno empujó al modelo en una dimensión particular: el Spa lo entrenó en la convención básica, el taxi le obligó a manejar concurrencia y agentes de software, la clínica le impuso densidad semántica, el banco le exigió bitemporalidad y trazabilidad regulatoria.
...
Antes de meternos en los escenarios, conviene aterrizar cómo se mapean los conceptos del ERP a las ocho coordenadas:
...
El próximo capítulo cambia radicalmente de territorio: pasamos del software empresarial al **sistema universitario**, donde lo que estresa al modelo no es la integración cross-módulo, sino el **timeline académico** (carreras compuestas por años, años por semestres, semestres por cursos), los **prerequisitos como grafo dirigido acíclico**, las **personas que cambian de rol a lo largo del tiempo** (un estudiante puede volverse docente; un docente, investigador; un investigador, tutor de tesis) y las **calificaciones que se asignan, se reclaman y a veces se rectifican**, generando un historial académico que dura décadas. Veremos cómo WQuestions absorbe ese mundo sin cambiar de motor.
