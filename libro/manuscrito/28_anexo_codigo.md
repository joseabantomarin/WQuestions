...
Este anexo recopila, en un solo lugar y por capítulo, **todos los fragmentos de código Python** que aparecen en la Parte V del libro (capítulos 15 al 20). El propósito es doble:
...
3. **El verbo se elige primero, los roles después.** Esto no es coincidencia: refleja la signatura tipada del verbo (capítulo 12). Primero el contrato, después los argumentos.
