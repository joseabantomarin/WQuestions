...
La corrección fue generalizar `instancia_de` a `V→K` —donde **V** es un comodín que significa "cualquier eje de valor"—. Clasificar es una operación universal: todo individuo de Q, O, L o T puede responder "¿de qué concepto soy instancia?". Y ese comodín `V` no resuelve solo este caso: es exactamente el mecanismo que el Capítulo 27 reclama para relajar otras signaturas demasiado estrechas (`paciente`, `partes`) de `O→Q` a `O→V`. La fricción, sometida a carga, no abrió un agujero en el modelo: lo empujó a alinearse con lo que ya afirmaba.
