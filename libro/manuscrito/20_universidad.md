# Capítulo 20 — Una universidad: timelines largos y grafos de dependencia
...
Antes de meternos en los casos, el mapeo a las ocho coordenadas:
...
El próximo capítulo cambia de territorio una vez más. Pasamos del sistema académico al **sistema municipal** — el gobierno local que gestiona licencias, permisos, denuncias, multas, expedientes administrativos. Lo que estresa al modelo allá no es ni la integración cross-módulo (que ya vimos en ERP) ni los grafos de dependencia académica, sino una pieza distinta: **la autoridad normativa que justifica cada acción del Estado**. Cada licencia se emite "porque tal ordenanza la autoriza"; cada multa se aplica "porque tal artículo del código la habilita"; cada denuncia abre un expediente cuyo curso está dictado por procedimientos formales. Veremos cómo D7 (las cuatro relaciones del "por qué") se vuelve indispensable cuando lo normativo está en cada paso.
